...
math_df = {
    ...
    "Formula/Equation/Symbol": ["Quadratic formula: $x = \\frac{{-b ± \\sqrt{{b^2 - 4ac}}}}{{2a}}$<br>Binomial theorem: $(a + b)^n = \\sum_{k=0}^{n} \\binom{n}{k} a^{n-k} b^k$<br>Exponential properties: $a^m \\cdot a^n = a^{m+n}$, $\\frac{{a^m}}{{a^n}} = a^{m-n}$",
                                 "Linear equation: $y = mx + b$<br>Quadratic equation: $ax^2 + bx + c = 0$<br>Exponential function: $y = ab^x$",
                                 "Pythagorean theorem: $a^2 + b^2 = c^2$<br>Sine, cosine, tangent definitions: $\\sin \\theta = \\frac{{\\text{{opposite}}}}{{\\text{{hypotenuse}}}}$, $\\cos \\theta = \\frac{{\\text{{adjacent}}}}{{\\text{{hypotenuse}}}}$, $\\tan \\theta = \\frac{{\\text{{opposite}}}}{{\\text{{adjacent}}}}$",
                                 "Area of a rectangle: $A = lw$<br>Area of a triangle: $A = \\frac{1}{2}bh$<br>Volume of a cylinder: $V = \\pi r^2 h$",
                                 "Mean: $\\bar{x} = \\frac{{\\sum x_i}}{{n}}$<br>Standard deviation: $s = \\sqrt{\\frac{{\\sum (x_i - \\bar{x})^2}}{{n-1}}}$<br>Probability: $P(E) = \\frac{{\\text{{number of favorable outcomes}}}}{{\\text{{total number of outcomes}}}}$",
                                 "Derivative: $\\frac{{df}}{{dx}}$<br>Integral: $\\int f(x) \\, dx$",
...
latex_math = df_math.to_latex(index=False)

# Enclose LaTeX code within HTML table tags
html_table = f"<table>{latex_math}</table>"

# Display LaTeX table using Streamlit
